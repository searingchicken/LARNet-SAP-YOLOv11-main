...
class DehazeFormer(nn.Module):
    def __init__(self, in_chans=3, out_chans=4, window_size=8,
        ...
        # 第一处：添加 Attention_Block 在 layer3 之后
        self.attention_block1 = Attention_Block(in_size=embed_dims[2], out_size=embed_dims[2])
        ...
        # 第二处：添加 Attention_Block 在 layer4 之后
        self.attention_block2 = Attention_Block(in_size=embed_dims[3], out_size=embed_dims[3])
    ...
    def forward_features(self, x):
        ...
        x = self.layer3(x)
        x = self.attention_block1(x)  # 第一处：应用在 layer3 之后

        x = self.patch_split1(x)
        x = self.fusion1([x, self.skip2(skip2)]) + x
        x = self.layer4(x)
        x = self.attention_block2(x)  # 第二处：应用在 layer4 之后
...
class Attention_Block(nn.Module):
    def __init__(self, in_size, out_size):
        ...
        mid_size = max(in_size // 2, 64)  # 64 for in_size=96, 64 for in_size=48 (since 48//2 < 64)
        split_size = max(out_size // 4, 16)  # 24 for out_size=96, 16 for out_size=48

        self.conv_1 = nn.Conv2d(in_size, mid_size, kernel_size=3, stride=1, padding=1)
        self.relu_1 = nn.LeakyReLU(0.2, inplace=True)
        self.conv_2 = nn.Conv2d(split_size * 3, mid_size, kernel_size=1, stride=1, padding=0)  # 72 or 48 channels
        self.relu_2 = nn.LeakyReLU(0.2, inplace=True)
        self.conv_3 = nn.Conv2d(mid_size, out_size, kernel_size=3, stride=1, padding=1)
        self.relu_3 = nn.LeakyReLU(0.2, inplace=True)

        self.conv_4 = nn.Conv2d(mid_size, mid_size, kernel_size=1, stride=1, padding=0)
        self.relu_4 = nn.LeakyReLU(0.2, inplace=True)

        self.conv_4_1 = nn.Conv2d(mid_size, split_size, kernel_size=3, stride=1, padding=1)
        self.relu_4_1 = nn.LeakyReLU(0.2, inplace=True)
        self.conv_4_2 = nn.Conv2d(mid_size, split_size, kernel_size=3, stride=1, padding=1)
        self.relu_4_2 = nn.LeakyReLU(0.2, inplace=True)
        self.conv_4_3 = nn.Conv2d(mid_size, split_size, kernel_size=1, stride=1, padding=0)
        self.relu_4_3 = nn.LeakyReLU(0.2, inplace=True)

        self.GSA_Block = GSA_Block(split_size, split_size)
        self.LPA_Block = LPA_Block()
        self.GDA_Block = GDA_Block(out_size, out_size)
    ...
    def forward(self, x):
        ...
        hx = torch.cat([GSA, LPA, hx3], dim=1)  # 72 channels (in_size=96) or 48 channels (in_size=48)
        hx = self.conv_2(hx)
        hx = self.relu_2(hx)
        ...
        hx = hx + self.identity(x)  # Final skip connection
        return hx
